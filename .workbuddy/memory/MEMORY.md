...
## 技术栈（锁定）
- Hexo 8.x + Butterfly 主题，Markdown 写作（位于 `blog/` 子目录）。
- 主页：纯静态 HTML/CSS/JS，位于仓库根目录 `index.html` + `assets/` + `en/`。
- 代码托管 GitHub；部署用 Cloudflare Pages（连 GitHub 自动上线，免费 + 自动 HTTPS）。
- 构建：根目录 `node scripts/build.js` 会先在 `blog/` 运行 Hexo 生成到 `public/blog/`，再把主页文件复制到 `public/`；最终 `public/` = 主页 + 博客。
- 本地预览：先运行 `node scripts/build.js` 生成 `public/`，再启动静态服务器（如 `cd public && python -m http.server 4000`）。
- 博客 UI 风格：简洁干练、单栏、无 banner/侧边栏/多余装饰，与主页同色系（#f8fafc 背景 + #4f46e5 点缀），自定义 CSS 在 `blog/source/css/custom.css`。
