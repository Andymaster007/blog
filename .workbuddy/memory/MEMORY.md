...
- 主页：纯静态 HTML/CSS/JS，位于仓库根目录 `index.html` + `assets/`。
- 代码托管 GitHub；部署用 Cloudflare Pages（连 GitHub 自动上线，免费 + 自动 HTTPS）。
- 构建：根目录 `node scripts/build.js` 会先构建博客到 `blog/public`，再合并到 `public/`（主页在根、`public/blog/` 是博客）。
- 本地预览：先运行 `node scripts/build.js` 生成 `public/`，再启动静态服务器（如 `python -m http.server 4000`）。
