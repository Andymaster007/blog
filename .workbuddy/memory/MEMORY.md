...
- 导航菜单（Fluid `navbar.menu`）：主页(`/`) / 项目(`/categories/项目/`) / 心得(`/categories/心得/`) / 归档(`/archives/`) / 分类(`/categories/`)。搜索= Fluid 原生 `search`（基于已装 hexo-generator-search 生成 `/search.xml`，导航栏自动出搜索框）。链接用根相对路径，Hexo 自动拼 `root:/blog/` 前缀。
- 主页：纯静态 HTML/CSS/JS，位于仓库根目录 `index.html` + `assets/` + `en/`。主页导航里的「Blog」按钮已加 `target="_blank"`，点它在**新标签页**打开博客（用户要求「打开新的窗口/新的网页，博客单独成页」）。
- 代码托管 GitHub；部署用 Cloudflare Pages（连 GitHub 自动上线，免费 + 自动 HTTPS）。
- 构建：根目录 `node scripts/build.js` 会先在 `blog/` 运行 Hexo 生成到 `public/blog/`，再把主页文件复制到 `public/`；最终 `public/` = 主页 + 博客。⚠️ 切换主题后若报 `entities`/`htmlparser2` 的 `exports` 冲突，执行 `cd blog && rm -rf node_modules pnpm-lock.yaml && pnpm install` 干净重装即可（pnpm 10.33.2 + hoisted linker）。
...
- 板块（categories）：两个顶级分类 → **项目**（实际做过的项目类文章，如 SDLSpaceShooter 打飞机游戏）、**心得**（平时学习中的心得体会类文章，由菜头帮总结整理）。子类/细分用 tags。旧的「算法心得 / 游戏项目 / 金融项目 / 编程技术杂谈」四级分类已弃用（SDLSpaceShooter 已从 `游戏项目` 改归 `项目`）。
